...
Http.prototype.start = function (cb) {
    ...
    if (this.useSSL) {
        this.server = https.createServer(this.sslOpts, this.http).listen(this.port, this.host, function () {
            self.logger.info(`Https server(${self.app.getServerId()}) started successfully, url: https://${self.host}:${self.port}`);
            process.nextTick(cb);
        });
    } else {
        this.server = http.createServer(this.http).listen(this.port, this.host, function () {
            self.logger.info(`Http server(${self.app.getServerId()}) started successfully, url: http://${self.host}:${self.port}`);
            process.nextTick(cb);
        });
    }
};
...
Http.prototype.afterStart = function (cb) {
    this.logger.info(this.useSSL ? 'Https' : 'Http' + `(${this.app.getServerId()}) server afterStart`);
    process.nextTick(cb);
};
...
Http.prototype.stop = function (force, cb) {
    ...
    this.server.close(function () {
        self.logger.info(self.useSSL ? 'Https' : 'Http' + `(${self.app.getServerId()}) server stop`);
        cb();
    });
};
